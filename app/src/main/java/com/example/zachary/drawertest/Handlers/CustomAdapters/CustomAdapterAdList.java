...
public class CustomAdapterAdList extends RecyclerView.Adapter<CustomAdapterAdList.Holder>{
    ArrayList<DataAd> dataAdList;
    ArrayList<String> Favourites = new ArrayList<>();
    boolean clickdirect;
    Context context;

    public CustomAdapterAdList(Context callingclass, ArrayList<DataAd> dataadlist, boolean movetoprofile) {
    ...
    public void onBindViewHolder(Holder tempholder, int itemnum) {
        ...
        if (favourited(dataAd.getadAdID())) { holder.favourite.setImageResource(R.drawable.ic_heart_red_full);}
        else {holder.favourite.setImageResource(R.drawable.ic_heart_red_empty);}
        ...
        holder.favourite.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                switchfavourites(holder, AdID);
            }
        });
    }

    public void switchfavourites(Holder holder, String adid) {
        if (!favourited(adid)) {
            holder.favourite.setImageResource(R.drawable.ic_heart_red_full);
            Favourites.add(adid);
        } else {
