...
import com.example.zachary.drawertest.Main.Fragments.PopUps.PopUpCalendar;
...
public class Register extends AppCompatActivity implements PopUpCalendar.CalendarPass, AdapterView.OnItemSelectedListener {
    ...
    String Email, Username, Password, PSE, Studying, Gender, BirthDate, CurrentDate;
    ...
    public void createaccount(View view) {
        ...
        if (master(Email, Username)) {
            ...
            dataUser.setUserProfile(
                    Username, Password, Email,
                    PSE, Studying, Gender, BirthDate,
                    ProfilePictureLarge, ProfilePictureSmall
            );

            /////////////////////////////////
            dataUser.setuserUserID("9");
            App.setUSRID("9");
            ...
            ServerInterface.sendUserNew(dataUser);
    ...
    private boolean master(String email, String username) {
        ...
        if (!matched(email)) {
            return false; }
        // Todo - Verify function textlength's necessity.
        if (!textlength()) {
            return false; }
        if (!usernamecheck(username)) {
            Toast.makeText(Register.this, "Username already taken. Please choose another.", Toast.LENGTH_LONG).show();
            return false; }
        else
            return true;
    }
    ...
    private boolean textlength(){
        int x = 25;
        int z = 3;
        if (etUsername.getText().toString().length() > x) {
            Toast.makeText(Register.this, "Username must be " + x + " or less characters", Toast.LENGTH_SHORT).show();
            return false; }
        if (etUsername.getText().toString().length() < z) {
            Toast.makeText(Register.this, "Username must be " + z + " or more characters", Toast.LENGTH_SHORT).show();
            return false; }
        else
            return true;
    }

    private boolean usernamecheck(String username) { return ServerInterface.loginCheckUsername(username); }
